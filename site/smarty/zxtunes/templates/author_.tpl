...
<img {if $author.dead}style="border: 4px solid black"{/if} src="{$scr}"></td><td >&#160;</td><td vAlign=top align=left>
...
{else}
<a class='Page' href="author.php?id={$author_id}&page={$pages[n]}">{$pages[n]}</a>
{/if}
{/section}
...
{else}
<a class='Page' href="author.php?id={$author_id}&page=all">
...
<img onclick="Rate({$playlist[n].id})" id="r{$playlist[n].id}" class="{$playlist[n].rt}" src="css/handup.gif" title="{if $language eq 'rus'}Мне нравится!{else}I Like it!{/if}"> <div style="display: inline" id="rn{$playlist[n].id}">{$playlist[n].rating}</div>
</td>
...
<div align='justify' style="FONT-SIZE: 1.2em; width: 540px">{$interview.text}</div>
<br>
	...
	<p><a href="/author.php?id={$author.id}&md=3&interview={$all_interview[n].id}">{$all_interview[n].title}</a></p>
	{/section}
	</div>
	{/if}
{else}
