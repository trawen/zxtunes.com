...
function init()
{
  ...
  $('#pl'+first_track).html("<div id='player'></div>");

  var params = {

	allowScriptAccess:"sameDomain", 
	allowFullScreen:"false", 
	track1:"fym2/"+author_id+"/"+first_track+".fym",
	volume:"0.8",
	autostart:"true",
	mixer:"YM ABC",
	movie:"player8.swf",
  ...
  swfobject.embedSWF("css/player8.swf", "player", "385", "11", "10.0", "expressInstall.swf", params, {bgcolor:params.background} );
...
function InsertPlayer(tr,au) {
  ...
  swfobject.embedSWF("css/player7.swf", "player", "385", "11", "10.0", "expressInstall.swf", params, {bgcolor:params.background} );
...
function play(new_track) {
	...
	if (last_track != new_track)	{
		...
		//$('#player').clone().appendTo('#pl'+new_track);
		...
		$('#pl'+last_track).empty();
		$('#pl'+new_track).html("<div id='player'></div>");
		InsertPlayer(new_track, author_id);
	
		// var zz = $('#pl'+new_track).html();
		// alert(last_track + " / " + new_track + " - " + zz);
		...
		author_id = $('#a'+new_track).text();
				...
				$('#pl'+last_track).addClass('plln');
		$('#pl'+new_track).removeClass('plln');
